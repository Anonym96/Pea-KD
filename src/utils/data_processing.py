...
"""
from utils.nli_data_processing import init_glue_model, init_glue_model_sim, init_glue_model_SPS, init_glue_model_SPS2, get_glue_task_dataloader, get_glue_task_dataloader_pretrain5
...
def init_model_sim(task_name, output_all_layers, num_hidden_layers, config):
    if 'race' in task_name.lower():
        return init_race_model(task_name, output_all_layers, num_hidden_layers, config)
    else:
        return init_glue_model_sim(task_name, output_all_layers, num_hidden_layers, config)
    
def init_model_SPS(task_name, output_all_layers, num_hidden_layers, config, shuffle= False):
    if 'race' in task_name.lower():
        return init_race_model(task_name, output_all_layers, num_hidden_layers, config)
    else:
        return init_glue_model_SPS(task_name, output_all_layers, num_hidden_layers, config, shuffle = shuffle)

def init_model_SPS2(task_name, output_all_layers, num_hidden_layers, config):
    if 'race' in task_name.lower():
        return init_race_model(task_name, output_all_layers, num_hidden_layers, config)
    else:
        return init_glue_model_SPS2(task_name, output_all_layers, num_hidden_layers, config)
