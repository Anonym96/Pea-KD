...
from BERT.pytorch_pretrained_bert.modeling import BertPreTrainedModel, BertModel, BertModel_sim, BertModel_SPS, BertModel_SPS_sim, BertModel_SPS2, BertEmbeddings, BertEncoder
...
class BertForSequenceClassificationEncoder_sim(BertPreTrainedModel):
    def __init__(self, config, output_all_encoded_layers=False, num_hidden_layers=None, fix_pooler=False):
        super(BertForSequenceClassificationEncoder_sim, self).__init__(config)
        if num_hidden_layers is not None:
            logger.info('num hidden layer is set as %d' % num_hidden_layers)
            config.num_hidden_layers = num_hidden_layers
        ...
        if fix_pooler:
            ...
            self.bert = BertModel_sim(config)
        self.output_all_encoded_layers = output_all_encoded_layers
        self.apply(self.init_bert_weights)
        
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        if self.output_all_encoded_layers:
            full_output, pooled_output, Qx, Kx, Vx = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True)
            return [full_output[i][:, 0] for i in range(len(full_output))], pooled_output, [Qx[i][:,0] for i in range(len(Qx))], [Kx[i][:,0] for i in range(len(Kx))], [Vx[i][:,0] for i in range(len(Vx))]
        else:
            _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
            return None, pooled_output
        
class BertForSequenceClassificationEncoder_SPS(BertPreTrainedModel):
    def __init__(self, config, output_all_encoded_layers=False, num_hidden_layers=None, fix_pooler=False, shuffle = False):
        super(BertForSequenceClassificationEncoder_SPS, self).__init__(config)
        if num_hidden_layers is not None:
            logger.info('num hidden layer is set as %d' % num_hidden_layers)
            config.num_hidden_layers = num_hidden_layers
        ...
        if fix_pooler:
            ...
            self.bert = BertModel_SPS(config, shuffle)
        self.output_all_encoded_layers = output_all_encoded_layers
        self.apply(self.init_bert_weights)
...
class BertForSequenceClassificationEncoder_SPS_sim(BertPreTrainedModel):
    def __init__(self, config, output_all_encoded_layers=False, num_hidden_layers=None, fix_pooler=False):
        super(BertForSequenceClassificationEncoder_SPS_sim, self).__init__(config)
        if num_hidden_layers is not None:
            logger.info('num hidden layer is set as %d' % num_hidden_layers)
            config.num_hidden_layers = num_hidden_layers
        ...
        if fix_pooler:
            ...
            self.bert = BertModel_SPS_sim(config)
        self.output_all_encoded_layers = output_all_encoded_layers
        self.apply(self.init_bert_weights)
...
class BertForSequenceClassificationEncoder_SPS2(BertPreTrainedModel):
    def __init__(self, config, output_all_encoded_layers=False, num_hidden_layers=None, fix_pooler=False, shuffle = False):
        super(BertForSequenceClassificationEncoder_SPS2, self).__init__(config)
        if num_hidden_layers is not None:
            logger.info('num hidden layer is set as %d' % num_hidden_layers)
            config.num_hidden_layers = num_hidden_layers
        ...
        if fix_pooler:
            ...
            self.bert = BertModel_SPS2(config)
        self.output_all_encoded_layers = output_all_encoded_layers
        self.apply(self.init_bert_weights)
