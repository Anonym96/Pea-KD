...
from utils.modeling import BertForSequenceClassificationEncoder, BertForSequenceClassificationEncoder_sim, BertForSequenceClassificationEncoder_SPS, BertForSequenceClassificationEncoder_SPS_sim, BertForSequenceClassificationEncoder_SPS2, FCClassifierForSequenceClassification
...
#         teacher_summary = f'~/KDAP/data/outputs/KD/RTE/RTE_patient_kd_teacher_12layer_result_summary.pkl'
#     elif task_name.lower() == 'mrpc':
#         teacher_summary = f'~/KDAP/data/outputs/KD/MRPC/MRPC_patient_kd_teacher_12layer_result_summary.pkl'
#     elif task_name.lower() == 'sst-2':
#         teacher_summary = f'~/KDAP/data/outputs/KD/SST-2/SST-2_patient_kd_teacher_12layer_result_summary.pkl'
#     elif task_name.lower() == 'qnli':
#         teacher_summary = f'~/KDAP/data/outputs/KD/QNLI/QNLI_patient_kd_teacher_12layer_result_summary_2.pkl'
    ...
    if types.lower() == 'train':
        ...
        dataset = TensorDataset(train_input_ids, PTP_label, train_input_mask, train_segment_ids)
    if types.lower() == 'train':
        dataloader = DataLoader(dataset, sampler = RandomSampler(dataset), batch_size = 64) 
    else : 
        dataloader = DataLoader(dataset, sampler = RandomSampler(dataset), batch_size = 64)
...
def init_glue_model_sim(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_sim(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier

def init_glue_model_SPS(task_name, output_all_layers, num_hidden_layers, config, shuffle = False):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_SPS(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers, shuffle= shuffle)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier

def init_glue_model_SPS_sim(task_name, output_all_layers, num_hidden_layers, config):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_SPS_sim(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier
...
def init_glue_model_SPS2(task_name, output_all_layers, num_hidden_layers, config, shuffle= False):
    logger.info('predicting for %s' % task_name.upper())
    processor = processors[task_name.lower()]()
    output_mode = output_modes[task_name.lower()]
    ...
    encoder_bert = BertForSequenceClassificationEncoder_SPS2(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers, shuffle = shuffle)
    classifier = FCClassifierForSequenceClassification(config, num_labels, config.hidden_size, 0)
    return encoder_bert, classifier
...
def init_pretrain_model_PTP_SPS(task_name, output_all_layers, num_hidden_layers, config, shuffle = False):
    logger.info('Initializing model for pretraining')
    mode = None
    num_labels_1 = 4
    print("-"*77)
    print("nli_data_processing line 986")
    print(shuffle)
    encoder_bert = BertForSequenceClassificationEncoder_SPS(config, output_all_encoded_layers=output_all_layers,
                                                        num_hidden_layers=num_hidden_layers, shuffle = shuffle)
    classifier_1 = FCClassifierForSequenceClassification(config, num_labels_1, config.hidden_size, 0)
