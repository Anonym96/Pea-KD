...
from utils.nli_data_processing import processors, output_modes, init_glue_model_SPS, init_glue_model_SPS2
...
from utils.utils import load_model, count_parameters, eval_model_dataloader_nli, eval_model_dataloader, load_model_real_wonbon, compute_metrics
...
if args.model_type == 'Original':
    pkd_layer_selection = 0
    if args.student_hidden_layers == 1:
        args.fc_layer_idx = None
    elif args.student_hidden_layers == 2:
        args.fc_layer_idx = '5'
    ...
    pkd_layer_selection = 1
    if args.student_hidden_layers == 1:
        args.fc_layer_idx = '5'
    elif args.student_hidden_layers == 2:
        args.fc_layer_idx = '2,5,8'
    elif args.student_hidden_layers == 3:
        args.fc_layer_idx = '1,3,5,7,9'
    elif args.student_hidden_layers == 6:
        args.fc_layer_idx = '3,7'    
...
if args.kd_model.lower() in ['kd', 'kd.cls', 'kd.u', 'kd.i']:
    ...
    # if SPS model for 1,2,3-layer student.
        ...
        student_encoder, student_classifier = init_glue_model_SPS(task_name, output_all_layers, args.student_hidden_layers, student_config, shuffle = False)
    elif args.model_type == 'SPS':
        student_encoder, student_classifier = init_glue_model_SPS(task_name, output_all_layers, args.student_hidden_layers, student_config, shuffle = True)
    
    n_student_layer = len(student_encoder.bert.encoder.layer)
    student_encoder = load_model_real_wonbon(student_encoder, args.encoder_checkpoint, args, 'student', verbose= True)
    logger.info('*' * 77)
...
if args.do_train:
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.kd_model in ['kd.u', 'kd.i']:
                full_output, pooled_output = student_encoder(input_ids, segment_ids, input_mask, args.kd_model)
            else :
            ...
            #teacher_pred_label = teacher_pred.argmax(1) 
            ...
            if (epoch == 8):
                logger.info('='*77)
                if task_name == 'cola':
                    logger.info("Best Loss : "+ str(eval_loss_min)+ "Best mcc : "+str(eval_best_acc))
                elif task_name == 'mrpc':
                    logger.info("Best Loss : "+ str(eval_loss_min)+ "Best F1 : "+str(eval_best_acc))
                else:
                    logger.info("Best Loss : "+ str(eval_loss_min)+ "Best acc : "+str(eval_best_acc))
                raise ValueError('Skip epoch 10')
