...
from utils.utils import load_model, count_parameters, eval_model_dataloader_nli, eval_model_dataloader, load_model_wonbon
...
task_fixed = args.task
student_hidden_layers_fixed = args.student_hidden_layers
train_type_fixed = args.train_type

if DEBUG:
    logger.info("IN DEBUG MODE")
    argv = get_predefine_argv(args, 'glue', task_name = task_fixed, train_type=train_type_fixed, student_layers = student_hidden_layers_fixed)    
    try:
...
if task_fixed is not None:
    args.task = task_fixed
    args.task_name = task_fixed
if student_hidden_layers_fixed is not None:
    args.student_hidden_layers = student_hidden_layers_fixed
...
# Specify where the teacher summary is saved in the below line. Below line is an example.
teacher_summary = os.path.join(HOME_OUTPUT_FOLDER, args.task+'/'+args.task+'_patient_kd_teacher_12layer_result_summary.pkl')
...
if args.model_type == 'Original':
    student_encoder, student_classifier = init_model(args.task_name, output_all_layers, args.student_hidden_layers, student_config)
...
if args.do_train:
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.train_type == 'finetune_student':
            ...
            if global_step % args.log_every_step == 0:
                print('{},{},{},{},{}'.format(epoch+1, global_step, step, tr_acc_1 / nb_tr_examples,
                      ...
                      file=log_train)
            # We stop the training after 3 epochs due to overfitting
            if (epoch == 2):
                logger.info("")
                logger.info('='*77)
